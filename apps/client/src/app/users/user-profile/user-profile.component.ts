...
import { imageValidationConfig } from '@cyber-sight/shared';
import { PasswordDialogComponent } from '~app/users/password-dialog/password-dialog-component';
...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  getFormFieldError(formControl: FormControl): string {
      ...
      update$.subscribe({
        ...
        error: (error) => {
          if (error.message) {
            ...
            this.snackbarService.somethingWentWrongMessage();
          }
        },
      });
    }
  ...
  deleteUser() {
    ...
    dialogRef
      ...
      .subscribe((conf) => {
        if (conf) {
          const passDialogRef = this.dialog.open(PasswordDialogComponent);
